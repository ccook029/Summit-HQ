...
//
//   - Accounting owner  → full access to the finance agents, the accounting
...
export async function getCurrentStaff(): Promise<StaffProfile | null> {
  ...
  if (id === SHARED_STAFF_ID) {
    return { id: SHARED_STAFF_ID, name: "Summit Owner", email: "" };
  }
  try {
    const dir = (await kv.get<Record<string, StaffProfile>>(DIRECTORY_KEY)) ?? {};
...
/** May this staff member use the finance agents and accounting console? */
