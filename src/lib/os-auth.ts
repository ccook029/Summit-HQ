// ---------------------------------------------------------------------------
// os-auth.ts — Summit OS staff session (docs/OS_LOGIN_DESIGN.md)
//
// One identity for Summit HQ. Tokens are compact HMAC strings
// (`os.<staffId>.<expiryEpoch>.<hmacSha256Hex>`) signed with the shared
// SUMMIT_OS_SESSION_SECRET. Today Summit uses the shared-passcode login
// (single owner); the id-based scheme leaves room for per-person staff
// logins later without changing the session model.
//
// Web Crypto only (no Node `crypto`) so the exact same code runs in the edge
// middleware and in Node route handlers.
// ---------------------------------------------------------------------------

export const OS_COOKIE = "summit_os_session";
export const OS_SESSION_TTL_SECONDS = 60 * 60 * 12; // 12h

/** Staff id used by the shared-passcode login (OS_SHARED_PASSCODE). */
export const SHARED_STAFF_ID = 0;

export function osAuthEnabled(): boolean {
  return Boolean(process.env.SUMMIT_OS_SESSION_SECRET);
}

async function hmacHex(payload: string): Promise<string> {
  const secret = process.env.SUMMIT_OS_SESSION_SECRET;
  if (!secret) throw new Error("SUMMIT_OS_SESSION_SECRET is not set");
