...
//
// Works for ANY department with a staffed manager: the boss reads their department context (live data)
...
//
// Per-department planning guidance lives in DISPATCH_INSTRUCTIONS.
// ---------------------------------------------------------------------------
...
const DISPATCH_INSTRUCTIONS: Record<string, string> = {
  finance:
    "Split the work by altitude: bookkeeping pieces to the Bookkeeper (categorize the uncategorized, reconcile named accounts, chase specific anomalies) and forward-looking pieces to the Financial Analyst (cash outlook, margins, spend review). Ground every brief in what the Zoho Books feed actually shows — if the feed is unavailable, dispatch at most one piece asking for the data path to be fixed rather than analysis on air.",
  operations:
    "Dispatch one piece per concrete thread: a vendor communication to draft, a commitment to chase, a status digest to produce. Every brief names the counterparty and the outcome wanted. Don't dispatch a piece whose key facts (who, what was agreed, by when) aren't in the brief or company knowledge — flag the gap instead.",
  bizdev:
    "Keep the funnel moving: a research piece to find fresh prospects in a named segment/geography, a qualification piece to score what research found, and an outreach piece to write first-touch emails to the HOT leads. Quality prospects that fit Summit's ICP beat a long list, and first-touch stays relational — one low-pressure ask, no pitch. If the ICP isn't defined in company knowledge yet, dispatch nothing and escalate that instead.",
};
...
export async function runDepartmentDispatch(
  ...
  const systemPrompt = `${
    ...
    `You are ${manager.name}, ${manager.title} at Summit, the boss of the ${dept.name} department.\n\nDEPARTMENT MISSION: ${dept.mission}`
  }
${knowledge}
