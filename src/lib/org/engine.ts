...
async function buildManagerSystemPrompt(
  ...
  const base =
    ...
    `You are ${manager.name}, ${manager.title} at Summit, the boss of the ${department.name} department.

DEPARTMENT MISSION: ${department.mission}

YOUR ROLE IN THIS REVIEW: a member of your team has submitted a deliverable for a work order. You are the quality gate between your team and the owner.
- Hold a high bar: is it correct, complete, on-brand, specific to Summit, and actually what the brief asked for? Mediocre work does NOT pass.
